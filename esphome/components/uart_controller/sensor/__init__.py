...
from ..const import CONF_CTRL_CHAR_TRIGGER, CONF_UART_CONTROLLER
...
def validate_control_character(config):
    if CONF_CTRL_CHAR_TRIGGER not in config:
        raise cv.Invalid(f" {CONF_CTRL_CHAR_TRIGGER} is a required property")
    if len(config[CONF_CTRL_CHAR_TRIGGER]) != 1:
        raise cv.Invalid(f" {CONF_CTRL_CHAR_TRIGGER} must be exactly one character")
    if (
        config[CONF_CTRL_CHAR_TRIGGER]
        not in "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ!?()[]}{<>&=;:.,*+-_@#"
    ):
        raise cv.Invalid(f" {CONF_CTRL_CHAR_TRIGGER} is using an invalid character")
