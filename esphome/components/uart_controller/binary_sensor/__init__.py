...
from ..const import CONF_CTRL_CHAR_OFF, CONF_CTRL_CHAR_ON, CONF_UART_CONTROLLER
...
def validate_control_character(config):
    if CONF_CTRL_CHAR_ON not in config:
        raise cv.Invalid(f" {CONF_CTRL_CHAR_ON} is a required property")
    if CONF_CTRL_CHAR_OFF not in config:
        raise cv.Invalid(f" {CONF_CTRL_CHAR_OFF} is a required property")
    if len(config[CONF_CTRL_CHAR_ON]) != 1:
        raise cv.Invalid(f" {CONF_CTRL_CHAR_ON} must be exactly one character")
    if len(config[CONF_CTRL_CHAR_OFF]) != 1:
        raise cv.Invalid(f" {CONF_CTRL_CHAR_OFF} must be exactly one character")
    if config[CONF_CTRL_CHAR_ON] == config[CONF_CTRL_CHAR_OFF]:
        raise cv.Invalid(f" {CONF_CTRL_CHAR_ON} must differ from {CONF_CTRL_CHAR_OFF}")
    if (
        config[CONF_CTRL_CHAR_ON]
        not in "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ!?()[]}{<>&=;:.,*+-_@#"
    ):
        raise cv.Invalid(f" {CONF_CTRL_CHAR_ON} is using an invalid character")
    if (
        config[CONF_CTRL_CHAR_OFF]
        not in "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ!?()[]}{<>&=;:.,*+-_@#"
    ):
        raise cv.Invalid(f" {CONF_CTRL_CHAR_OFF} is using an invalid character")
