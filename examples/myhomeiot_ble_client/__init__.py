from esphome import automation, const
...
async def to_code(config):
    ...
    if len(config[CONF_SERVICE_UUID]) == len(esp32_ble_tracker.bt_uuid16_format):
        ...
        uuid128 = (
            esp32_ble_tracker.as_reversed_hex_array(config[CONF_SERVICE_UUID])
            if reversed
            else esp32_ble_tracker.as_hex_array(config[CONF_SERVICE_UUID])
        )
    ...
    if len(config[CONF_CHARACTERISTIC_UUID]) == len(esp32_ble_tracker.bt_uuid16_format):
        ...
        uuid128 = (
            esp32_ble_tracker.as_reversed_hex_array(config[CONF_CHARACTERISTIC_UUID])
            if reversed
            else esp32_ble_tracker.as_hex_array(config[CONF_CHARACTERISTIC_UUID])
        )
    ...
    for conf in config.get(CONF_ON_VALUE, []):
        ...
        await automation.build_automation(
            ...
            [
                ...
                (MyHomeIOT_BLEClientConstRef, "xthis"),
            ],
            conf,
        )
